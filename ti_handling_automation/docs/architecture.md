...
- **Gateway session-affinity cookie.** Trackspace can sit behind an Azure Application Gateway with cookie-based affinity that `307`-redirects the first cookieless request to plant `ApplicationGatewayAffinity`/`JSESSIONID` (observed on INT). A `Prime_Affinity_Cookie` GET (its `307` tolerated) captures that `Set-Cookie`; `Build_Cookie_Parts` reduces it to its `name=value` parts; every Jira HTTP call then sends `Cookie: @{join(body('Build_Cookie_Parts'), '; ')}` so the gateway serves the pinned backend instead of looping on `307`. Because the `Set-Cookie` carries the `sentinelsvc` session (`JSESSIONID`), and Jira can set a new one on any call (primer failed, timed out or returned none; run moved to another cluster node), every Trackspace HTTP call (the primer included) and `Build_Cookie_Parts` secure their inputs **and** outputs, and there is no cookie variable (variable actions cannot be secured). Empty/harmless where a gateway doesn't use affinity.
...
- **Secrets flagged `secureData`** on `Get_Jira_password`, `Build_Cookie_Parts` and every Trackspace HTTP call, each securing inputs **and** outputs: the Basic auth header and `Cookie` going out, and any `sentinelsvc` `JSESSIONID` Jira sets coming back. Run history therefore shows the Jira calls by status only, without Jira's status code, headers or body. AbuseIPDB has no `secureData` flag because the key never enters the workflow.
