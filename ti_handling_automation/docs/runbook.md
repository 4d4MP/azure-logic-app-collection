...
Every Trackspace HTTP action secures its inputs and outputs (see
[07 — Gateway session affinity](07-ti-handler-playbook.md#gateway-session-affinity-the-cookie-header)),
so run history shows a failed Jira call only as Failed, without Jira's status code, headers or
body. The run's error message still carries the status where the playbook captures it (clone,
transition POST). For any other Jira error text in the rows below, repeat the failing call by
hand as `sentinelsvc`.
...
| `Resolve_Template_Id` / `Clone_OPSLSY_Change` fail with `401` | Trackspace password rotated | Run history shows these calls only as Failed: their status code and body are secured (`Terminate_Clone_Failed`'s message still names the clone's status). To see the real status, call `GET {JIRAHOST}/rest/api/2/myself` by hand with the `sentinelsvc` password from KV; if that returns `401`, update the `sentinelsvc` secret in KV. |
| Jira calls fail with `307` (loop) | Gateway session-affinity cookie not applied | `Prime_Affinity_Cookie` failed to capture `Set-Cookie`, so the `Cookie` header built from `Build_Cookie_Parts` is empty. Every Trackspace call secures its inputs and outputs (Jira can set the `sentinelsvc` `JSESSIONID` on any of them), so run history hides the `307` status code and body as well as the headers: the primer's `Set-Cookie` and the Jira calls' own response headers. The actions only show Failed. Call `GET {JIRAHOST}/rest/api/2/serverInfo` (or `/rest/api/2/myself`) by hand to see the real status and the gateway's `Set-Cookie`; confirm the gateway still uses `ApplicationGatewayAffinity` cookies and the `Build_Cookie_Parts` split still matches their `Set-Cookie` shape. |
