...
infrastructure, then the function code is published separately. The script below does
both.
...
Save this next to the files it names and run it from that directory — it takes no
paths, so copying the artifacts flat into a working directory (a Cloud Shell folder,
say) is all the setup there is:
...
publish overwrites in place. `RG` and `SUBSCRIPTION` can be overridden from the
environment; everything else comes from `azuredeploy.parameters.json` and the
deployment's own outputs, so the script cannot drift from the template.
...
`./deploy.sh --grant` does 1–3 against the identities the deployment just created.
Without `--grant` the script prints the three commands with the real object ids
